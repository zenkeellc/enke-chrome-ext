...
async function handleLogin() {
  ...
  // Listen for auth state changes from background SW
  storageListener = (changes: Record<string, chrome.storage.StorageChange>) => {
    const change = changes['enke_user_state'];
    if (change?.newValue?.isLoggedIn && change.newValue?.tokens?.token) {
      // Auth complete — clean up and emit success
      if (storageListener) {
  ...
  try {
    ...
    // Safety timeout: if auth doesn't complete within 30s, stop loading
    setTimeout(() => {
      if (loading.value) {
        if (storageListener) {
          chrome.storage.onChanged.removeListener(storageListener);
